...
\end{itemize}
The scanner strongly flags close companions, dual-nucleus candidates, and late-stage mergers where tidal bridges might fall below the $7.0\sigma$ threshold while the distinct cores remain visible. Of the 605 flagged multi-component candidates, quantitative analysis of the secondary flux ratios reveals:
\begin{itemize}
    \item \textbf{367 major merger candidates} (secondary component flux is $\ge$ 50\% of the primary)
    \item \textbf{238 minor companion candidates} (secondary component flux is between 15\% and 50\%)
\end{itemize}
...
To validate the deterministic scanner, the pipeline automatically generates bounding box overlays for the highest-scoring candidates. As shown in Figure \ref{fig:overlays}, these overlays visually confirm the pipeline's ability to distinguish the primary central target (red square) from nearby, distinct luminous secondary sources (cyan squares) that survive the strict thresholding and separation criteria.
