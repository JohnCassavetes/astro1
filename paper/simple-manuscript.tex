...
\begin{itemize}
    ...
    \item \textbf{The Result:} We generated a highly reliable ``hit list'' of 605 galaxy mergers for astronomers to study in detail.
\end{itemize}
...
First, it finds the main, bright blob in the center of the picture (the primary galaxy). Then, it scans the immediate surrounding area. It is hunting for a second distinct, bright blob (a ``buddy''). If it finds a buddy that is bright enough and close enough, it flags the image as a multi-component system. This is smoking-gun evidence of a galaxy collision or a massive pair orbiting each other.

\section{What We Found} \label{sec:results}

Our pipeline processed the 4,690 images totally autonomously. In the end, the image scanner successfully flagged \textbf{605 galaxies} showing strong evidence of having a companion or being in the middle of a merger. 
...
\begin{itemize}
    \item \textbf{367 Major Mergers:} The buddy galaxy was huge (at least 50\% as bright as the main one).
    \item \textbf{238 Minor Companions:} The buddy galaxy was smaller (between 15\% and 50\% as bright).
\end{itemize}
...
We have taken millions of boring data points and distilled them into a highly reliable, prioritized ``hit list'' of 605 complex, colliding galaxies. Astronomers can now take the top 20 candidates from our list and immediately request time on billion-dollar facilities like the James Webb Space Telescope or the Keck Observatory to study them in incredible detail, saving hundreds of hours of manual searching.
