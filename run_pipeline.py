...
def run_stage(script_path: str, stage_name: str, args: list = None) -> bool:
    print(f"\n{'='*60}")
...
def main():
    ...
    for script_path, stage_name in STAGES:
        ...
        if "download_data" in script_path:
            if args.skip_download:
                print(f"Skipping {stage_name} as requested.")
                continue
            cmd_args = ["--n", str(args.limit)]
            
        success = run_stage(script_path, stage_name, cmd_args)
        if not success:
    ...
    print("Results are available in the results/final directory.")
    print("="*60)
